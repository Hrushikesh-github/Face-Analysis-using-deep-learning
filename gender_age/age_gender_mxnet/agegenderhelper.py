...
class AgeGenderHelper:
    def __init__(self, config):
        # store the configuration object and build the age bins used for constructing class labels
        self.config = config
    ...
    def toGenderLabel(self, gender):
        # return 0 if the gender is male or else 1 for female
    ...
    def buildOneOffMappings(self, le):
        ...
        classes = sorted(le.clasees_, key=lambda x: int(x.decode("utf-8").split("_")[0]))
        oneOff = {}
        ...
        for foldPath in foldPaths:
            ...
            for row in rows:
                ...
                if age[0] != "(" or gender not in ("m", "f"):
                    #print("Found a incorrect value, ignoring it")
                    i += 1
                    continue
                ...
                # if the label is None, then the age does not fit into our age brackets, ignore the sample
                if label is None:
                    continue

                # update the respective image paths and label lists
